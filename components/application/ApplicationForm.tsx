...
const STEPS = [
  { title: "Identity", icon: <Mountain className="h-5 w-5" />, description: "Begin", microcopy: "Who are you becoming?" },
  { title: "Calling", icon: <Heart className="h-5 w-5" />, description: "Feel", microcopy: "What pulls you higher?" },
  { title: "Path", icon: <Map className="h-5 w-5" />, description: "Map", microcopy: "Where have your steps led?" },
  { title: "Summit", icon: <Star className="h-5 w-5" />, description: "Rise", microcopy: "Are you ready to ascend?" }
];
      ...
      >
        ...
        <div className="p-6 min-h-[350px] flex flex-col justify-center">
          <AnimatePresence mode="wait">
            <motion.div key={currentStep} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} transition={{ duration: 0.4 }}>
              {renderStep()}
            </motion.div>
          </AnimatePresence>
        </div>
