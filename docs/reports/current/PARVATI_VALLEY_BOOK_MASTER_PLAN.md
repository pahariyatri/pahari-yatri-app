...
Kalga and Pulga are done (published this session, each with dual traveller-perspective stories — an Indian voice and a foreign voice for Kalga, a remote-worker voice for Pulga, directly responding to the ask for local-Indian and international perspectives rather than one narrator archetype repeated). Remaining order: **Tosh** next (already researched) → **Kheerganga** (port/redo the existing research and reparent, given it already has real search demand) → **Grahan** → **Malana** (needs the most editorial care) → then Manikaran Sahib and Mantalai Lake as the two "add more chapters" candidates worth pursuing.
