...
| 4 | Tosh | **Researched, not written** — altitude, history and "hippie heaven" reputation sourced (Wikipedia + travel press); the "Tibetan cultural influence" claim needs a second, stronger source before stating as fact | Real demand unknown, worth checking before writing | Needs sourcing — do not use generic stock |
| 5 | Kalga | **Researched, not written** — altitude (~2,280 m), village size, apple-orchard economy sourced across two independent outlets | Same | Needs sourcing |
| 6 | Pulga | **Researched, not written** — altitude, the "Hummus Trail" phenomenon sourced across three independent outlets (ThePrint, Onmanorama/IANS, Business Standard) | Same | Needs sourcing |
...
Matches the brief's own priority list, adjusted for what's actually publishable next: **Kalga and Pulga** (both already researched, geographically adjacent, natural pair) → **Tosh** → **Kheerganga** (port/redo the existing research and reparent, given it already has real search demand) → **Grahan** → **Malana** (needs the most editorial care) → then Manikaran Sahib and Mantalai Lake as the two "add more chapters" candidates worth pursuing.
