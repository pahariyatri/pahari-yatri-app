...
| 8 | Kheerganga | **Published** (reparented `kheerganga-buni-buni-pass`) — ported the verified research from the other branch, including the camping-ban correction (banned since July 2024); the linked story `kheerganga-fire-and-water.mdx` also had its unhedged legend line fixed | Real, measurable demand exists (`kheerganga trek`, `kheerganga history` queries) — the one chapter in this list with actual search traffic already | Existing chapter photo |
...
| 10 | Grahan | **Published** (`grahan-protects-its-traditions`) — re-researched this session specifically to resolve the two previously-held claims. Alcohol ban: now cross-confirmed across independent sources including a Tripadvisor forum thread; stated as fact, with its deity-vision origin kept as tradition, not history. Zero cell signal: similarly cross-confirmed, stated as fact. The "only village in India" superlative is still declined — that specific claim remains unconfirmed | Unknown | Existing chapter photo |
| 11 | Rasol | **Not published** — partial coverage inside `kasol-weekend`, not researched as a standalone place | Unknown | Unknown |
| 12 | Malana | **Published** (`malana-myth-and-reality`) — the most heavily hedged chapter in the set. Alexander's-army origin and "oldest democracy" claims both repeated with their unproven/unsettled caveats intact, never stated as fact. The real cannabis economy is acknowledged (a documentary that ignored it would be dishonest) but handled deliberately: no access information, no product naming, framed around a failed government crop-substitution program and the cost to local youth rather than tourist curiosity. Confirmed live that "Malana Cream" does not appear anywhere on the published page | Real query volume exists for "malana himachal" broadly, though not measured in this site's own GSC data | Existing chapter photo; this is the village most likely to attract clickbait imagery online, avoid anything in that style |
